...
export const getSimilarMunicipalities = async (
  code: string
): Promise<Array<Municipality>> => {
  try {
    const data: Array<Municipality> = await api.GET(
      `municipality/similar/${code}`
    );
    return data;
  } catch (e) {
    console.log(e);
...
export const getMunicipalityInfo = async (
  code: string
