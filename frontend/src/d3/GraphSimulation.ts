...
export default class {
  ...
  addData = (ontologies: Array<Ontology>, clickedNode: GraphNode) => {
    ...
    this.unfilteredNodes = this.unfilteredNodes
      .concat(
        ontologies.map(common.mapOntologyToNonClickedGraphNode(clickedNode))
      )
      .filter(common.removeDuplicates)
      .filter(common.removeUndefinedTypes)
      .map(
        common.mapNodeToGraphNodeAtDefaultPosition(clickedNode.x, clickedNode.y)
      );

    this.unfilteredEdges = this.unfilteredEdges
      .concat(
        ontologies
          .map(common.makePredicateUnique)
          .map(common.mapOntologyToGraphEdge)
      )
      .filter(common.removeDuplicates)
  ...
  private removeNode = (node: GraphNode) => {
    ...
    if (common.removingNodeWillMakeGraphEmpty(node, this.edges)) {
      reduxStore.dispatch(
        setError(new Error("Oops, this would remove all nodes from the graph"))
      );
      return;
    }
  ...
  private drawEdges = () => {
    this.edgeSvg
      ...
      .join((enter) => {
        ...
        g.append("text")
          .text((edge) =>
            common.createEdgeLabelText(edge.sourceToTarget, false)
          )
  ...
  private showNodeMenu = async (
    ...
    const menuG = g
      ...
      .attr(
        "transform",
        `translate(${[menuPos.x, menuPos.y]}) scale(${menuPos.scale})`
      );
  ...
  drawEdgeLabels = () => {
    ...
    const edgeLabelFontSize = helper.getEdgeLabelFontSize(
      fontSize,
      maxEdgeFontSize,
      this.scale
    );
    ...
    this.nodeSvg
      ...
      .join((enter) => {
        ...
        g.append("circle")
          ...
          .attr("fill", (node) =>
            common.updateColorKPI(node.type, this.kpiToggle)
          )
          .attr("stroke", "#aaa")
        ...
        g.append("image")
          ...
          .attr(
            "transform",
            `translate(${-nodeRadius / 2.4},${nodeRadius / 4.0})`
          )
  ...
  private updateEdgePositions = () => {
    ...
    g.selectChild(helper.selectNodeOrEdge)
      .attr(
        "x1",
        (edge: any) => edge.source.x - (edge.source.x + edge.target.x) / 2
      )
      .attr(
        "y1",
        (edge: any) => edge.source.y - (edge.source.y + edge.target.y) / 2
      )
      .attr(
        "x2",
        (edge: any) => edge.target.x - (edge.source.x + edge.target.x) / 2
      )
      .attr(
        "y2",
        (edge: any) => edge.target.y - (edge.source.y + edge.target.y) / 2
      );
    ...
    g.selectChild(helper.selectEdgeLabel1).each(function (edge) {
      ...
      const text = common.createEdgeLabelText(
        edge.sourceToTarget,
        position.flip
      );
  ...
  private drawNodeMenuButton = (
    ...
    button
      ...
      .on("mouseover", function highlightMenu() {
        d3.select(this)
          ...
          .style("stroke", edgeHighlightColor);
      })
    ...
    button
      ...
      .attr(
        "transform",
        `translate(${-nodeMenuBtnRadius},${-nodeMenuBtnRadius})`
      )
  ...
  private redrawGraphWithFilter = () => {
    this.nodes = this.unfilteredNodes
      .filter(this.nodeFilter)
      .filter(this.filterKPISetSelection);
  ...
  */
  ...
  private unlockNodePosition = (
    ...
    d3.select(nodeContainer)
      .selectChild(helper.selectNodeLockIcon)
      .style("opacity", 0);
    ...
    this.svg.call(
      d3
        ...
        .on("zoom", (event: d3.D3ZoomEvent<SVGSVGElement, any>) => {
          ...
          this.nodeSvg.attr(
            "transform",
            `translate(${translate}) scale(${this.scale})`
          );
          this.edgeSvg.attr(
            "transform",
            `translate(${translate}) scale(${this.scale})`
  ...
  private scaleGraph = () => {
    ...
    nodes
      ...
      .attr("stroke-width", nodeStrokeWidth / this.scale);
      ...
      .call(
        d3
          ...
          .on("end", (event, d) => {
            ...
            const menu = (event.sourceEvent.target as SVGElement)
